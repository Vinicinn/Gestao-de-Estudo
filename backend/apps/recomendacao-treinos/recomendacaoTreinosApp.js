...
export async function recomendacaoTreinosApp() {
  const database = await getDatabase("recomendacaoTreinos");
  ...
  router.use("/workouts", authMiddleware, resourceRoutes(resourceController));
  router.use("/profile", authMiddleware, scheduleRoutes(scheduleController));
